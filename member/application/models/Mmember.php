...
class Mmember extends CI_Model
{
    ...
    public function login($inputan)
    {
        ...
        $this->db->where('is_verified', 1); // Pastikan akun sudah diverifikasi
        $q = $this->db->get('member');
        $cekmember = $q->row_array();

        //jika kosong, maka ada
        if (!empty($cekmember)) {
            //membuat tiket
    ...
    function ubah($inputan, $id_member)
    {
        ...
        //jika password tidak kosong, maka password di enkripsi
        ...
        //karena akun telah di ubah, maka tiket harus buat baru
    ...
    // FUNGSI BARU: Cari member berdasarkan token
    function find_by_token($token)
    {
        return $this->db->get_where('member', ['verification_token' => $token])->row();
    }

    // FUNGSI BARU: Aktivasi akun member
    function activate_account($id_member)
    {
        $payload = [
            'is_verified' => 1,
            'verification_token' => null
        ];
        $this->db->where('id_member', $id_member); // Asumsi primary key adalah 'id_member'
        return $this->db->update('member', $payload);
    }
}
